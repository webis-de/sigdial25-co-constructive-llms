...
def run_experiment(df, folds_dict, label_clm, input_clm, output_dir, model_name='bert-base-uncased', num_train_epochs=5, eval_steps=500, lr=2e-6, batch_size=4, val_size=0.2):
    ...
    for i in range(5):
        if os.path.exists(output_dir + 'fold-{}'.format(i)):
            continue
...
if __name__=="__main__":
    ...
    parser.add_argument('--output_path', type=str, default="../../data/turn-label-models")
    ...
    eli5_annotation_df     = load_ds('../../data/eli5_ds/annotation-results/MACE-measure/final_mace_predictions_training.pkl', args.model_name)
    ...
    for ds in ['all']:
        ...
        eval_results = run_experiment(working_dlgs_df, all_folds, label_clm, args.input_clm, output_path, num_train_epochs=args.num_train_epochs, model_name=args.model_name, eval_steps=500, lr=2e-6, batch_size=batch_size)
